...
public class ProcessingTask extends TimerTask {
	...
	public ProcessingTask(long reqId, AtomicLong concurrentRequests, int processingTimeMs,
	...
	public void run() {
		...
		if (deferredResult.isSetOrExpired()) {
			LOG.info("{}: Processing of non-blocking request #{} already expired", concReqs, reqId);
		} else {
			boolean deferredStatus = deferredResult.setResult(new ProcessingStatus("Ok", processingTimeMs));
			LOG.info("{}: Processing of non-blocking request #{} done, deferredStatus = {}", concReqs, reqId,
					deferredStatus);
		}
	}
}